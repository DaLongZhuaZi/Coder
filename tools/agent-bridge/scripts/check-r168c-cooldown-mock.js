...
const TOKEN = '123456';
const NONCE = 'r168c-nonce-' + Date.now();
const WS_URL = 'ws://127.0.0.1:8788/ws?token=' + TOKEN + '&clientId=r168c&appNonce=' + NONCE;

class Rpc {
  constructor(conn) { this.conn = conn; this.seq = 0; this.pending = new Map(); }
  call(type, payload) {
    const id = 'r168c_' + String(++this.seq);
    return new Promise((resolve, reject) => {
      this.pending.set(id, { resolve, reject });
      this.conn.sendJson({ id, type, payload: payload || {} });
      setTimeout(() => { if (this.pending.has(id)) { this.pending.delete(id); reject(new Error('timeout ' + type)); } }, 10000);
    });
  }
}
...
async function main() {
  ...
  await rpc.call('hello', { endpoint: 'ws://127.0.0.1:8788/ws', clientId: 'r168c', appNonce: NONCE, appVersion: '1.0.0' });

  const GONE = 'mock:missing-session-' + Date.now();
  for (let i = 1; i <= 3; i++) {
    const t0 = Date.now();
    try {
      const r1 = await rpc.call('session.messages', { sessionId: GONE });
      console.log('query', i, 'in', Date.now() - t0, 'ms ->', JSON.stringify(r1).slice(0, 150));
    } catch (e) {
      console.log('query', i, 'in', Date.now() - t0, 'ms ERROR:', e.message);
    }
  }
