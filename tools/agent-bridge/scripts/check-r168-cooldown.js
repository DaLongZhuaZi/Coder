...
const TOKEN = '123456';
const NONCE = 'r168-nonce-' + Date.now();
const WS_URL = 'ws://127.0.0.1:8788/ws?token=' + TOKEN + '&clientId=r168-cd&appNonce=' + NONCE;

class Rpc {
  constructor(conn) { this.conn = conn; this.seq = 0; this.pending = new Map(); }
  call(type, payload) {
    const id = 'r168_' + String(++this.seq);
...
async function main() {
  ...
  await rpc.call('hello', { endpoint: 'ws://127.0.0.1:8788/ws', clientId: 'r168-cd', appNonce: NONCE, appVersion: '1.0.0' });

  const GONE = 'codex:019f51a7-7362-7032-84e9-6a5d9ff4a641';
  let t0 = Date.now();
  try {
    const r1 = await rpc.call('session.messages', { sessionId: GONE });
    console.log('first query in', Date.now() - t0, 'ms ->', JSON.stringify(r1).slice(0, 200));
  } catch (e) {
    console.log('first query in', Date.now() - t0, 'ms ERROR:', e.message);
  }
  t0 = Date.now();
  try {
    const r2 = await rpc.call('session.messages', { sessionId: GONE });
    console.log('second query in', Date.now() - t0, 'ms ->', JSON.stringify(r2).slice(0, 200));
  } catch (e) {
    console.log('second query in', Date.now() - t0, 'ms ERROR:', e.message);
  }
